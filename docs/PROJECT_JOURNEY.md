...
> 记录每一次。
## 2026年1月30日：web仪表盘新增云网络概念演示平台，和混合云平台，并且新增物理设备的档案卡片
- **优化目标**：引入云网络，对接真实的vpc,并且利用模拟云网络平台（自己搭建的）来抽象查询vpc,安全组等云资源的能力
- **技术**：
  - 1.在测试物理设备云资源的统一模型时需要pytest框架，对接真实的云网络时需要Mock模块对所需要依赖的外部模块进行造假
  - 2.创建数据模型时类的继承（这里有点绕）
- **主要改动**：
  - 1.新增：config/cloud_resources.yaml 用来存放所有的模拟和真实云资源，让数据统一模型里面的档案卡来读取
  - 2.修改core/health_check/health_checker.py，nornir_tasks.py，统一两者的返回格式和样式与档案卡连接
  - 3.新增: core/cloud/real_providers/ali_client.py文件，对自己真实创建的VPC进行信息查询
- **学到的知识**：
  - 1.VPC 就是你在云上的「专属网络地盘」，里面的东西和你自己公司机房里的网络组件一一对应，只不过都是云厂商给你虚拟出来的 “专属使用权”，你远程配置就行，
  - 2.SDK 的全称是 Software Development Kit（软件开发工具包），你可以把它理解成：
云厂商（阿里云）为了让你能轻松操控自己的 VPC 封地，专门给不同编程语言（Python/Java/Go 等）定制的「标准化传令手册」。
- **提交哈希**：`944cefd `（代码回溯：git checkout `944cefd ` ,git checkout --detach `944cefd `，git checkout main（取消回溯））
