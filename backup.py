...
def backup_single_device(device_info):
    print(f"正在尝试连接{device_info['host']}......")
    connections = None
    try:
        connections = ConnectHandler(**device_info)
        print("连接成功！")
        print(f"正在开始备份设备{device_info['host']}........请稍后....")
        ...
        with open(filename, "w", encoding="utf-8") as f:
            f.write(output)
        print(f"设备{device_info['host']}备份成功！")
        print(f"备份之后的路径\文件名为：{filename}")
        return True
...
def main():
    ...
    target_devices = []
    if args.all:
        print(f"模式：备份已经读取的所有设备！ 共{len(devices)}台设备")
        target_devices = devices
    elif args.ip:
        print(f"模式：指定一个设备或者多个设备开始备份")
        for ip in args.ip:
            match = [d for d in devices if d["host"] == ip]
            if match:
                print(f"已经将{ip}加入到备份列表当中！")
